...
class WindowControlsIndicator extends PanelMenu.Button {
    ...
    this._iconsRootPath = `${this._iconPath}/icons`;
        ...
        // Panel titlebar behaviour: drag to move, double-click to toggle maximize,
        ...
        this._panelPressId = Main.panel.connect(
            'button-press-event',
            this._onPanelButtonPress.bind(this)
        );
    }

    // Returns true when the click landed on an empty panel area (not on any
    // interactive child widget such as a button or indicator).
    _isPanelBackground(source) {
        return source === Main.panel ||
               source === Main.panel._leftBox ||
               source === Main.panel._centerBox ||
               source === Main.panel._rightBox;
    }

    _onPanelButtonPress(actor, event) {
        const focusWindow = global.display.focus_window;
        if (!focusWindow) return Clutter.EVENT_PROPAGATE;

        if (!this._isPanelBackground(event.get_source()))
            return Clutter.EVENT_PROPAGATE;
        ...
        if (button === 1) {
            if (event.get_click_count() === 2) {
                ...
                if (focusWindow.maximized_horizontally && focusWindow.maximized_vertically) {
                    try { focusWindow.unmaximize(); }
                    catch (_) { focusWindow.unmaximize(Meta.MaximizeFlags.BOTH); }
                } else {
                    try { focusWindow.maximize(); }
                    catch (_) { focusWindow.maximize(Meta.MaximizeFlags.BOTH); }
                }
                return Clutter.EVENT_STOP;
            }

            // Single press — arm the drag tracker
            const [x, y] = event.get_coords();
            this._dragWindow = focusWindow;
            this._dragStartX = x;
            this._dragStartY = y;
            this._dragThresholdMet = false;
    ...
    _onStageMotion(actor, event) {
        if (!this._dragWindow || this._dragThresholdMet)
            return Clutter.EVENT_PROPAGATE;
        ...
        if (dx > 8 || dy > 8) {
            ...
            // Pin the target window before anything can shift focus.
            const targetWindow = this._dragWindow;

            // Explicitly unmaximize before calling begin_grab_op.
            // If we let begin_grab_op do it implicitly, Mutter shifts focus to
            // whatever window happens to be under the topbar at that moment, and
            // the grab op then latches onto that window instead of the one we
            // pressed on. By unmaximizing first and then deferring begin_grab_op
            // one tick, we let the window settle into its restored geometry so
            // Mutter picks it up correctly.
            const isMaximized = targetWindow.maximized_horizontally &&
                                 targetWindow.maximized_vertically;
            if (isMaximized) {
                try { targetWindow.unmaximize(); }
                catch (_) {
                    try { targetWindow.unmaximize(Meta.MaximizeFlags.BOTH); }
                    catch (_e) {}
                }
            }

            // Remove stage listeners before the grab op takes pointer ownership.
            this._cleanupPanelDrag();

            // Defer one tick so Mutter can finish updating the window's frame
            // rect after unmaximize before we start the move grab op.
            const capturedX = x;
            const capturedY = y;
            const capturedTime = event.get_time();
            const capturedState = event.get_state();
            GLib.idle_add(GLib.PRIORITY_DEFAULT, () => {
                if (!targetWindow || targetWindow.is_destroyed?.())
                    return GLib.SOURCE_REMOVE;
                try {
    ...
    _onStageButtonRelease(_actor, _event) {
        this._cleanupPanelDrag();
        return Clutter.EVENT_PROPAGATE;
    }
    ...
    _updateVisibility() {
        ...
        // Show buttons for any focused window (not just maximized/fullscreen)
    ...
    destroy() {
        if (this._panelPressId) { Main.panel.disconnect(this._panelPressId); this._panelPressId = null; }
        this._cleanupPanelDrag();
